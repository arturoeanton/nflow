...
func main() {
	...
	gNFlow.GET("/templates", func(c echo.Context) error {
		db, err := playbook.GetDB()
		if err != nil {
			log.Println(err)
		...
		defer conn.Close()
		rows, err := conn.QueryContext(ctx, playbook.Config.DatabaseNflow.QueryGetTemplates)
		if err != nil {
			log.Println(err)
			return c.JSON(http.StatusNotFound, map[string]interface{}{"message": "Not Found"})
	...
	gNFlow.GET("/templates/:name", func(c echo.Context) error {
		name := c.Param("name")
		template := playbook.GetTemplateFromDB(name)

		content := template
		return c.JSON(http.StatusOK, map[string]interface{}{"name": name, "content": content})
	})

	gNFlow.POST("/templates", func(c echo.Context) error {
		...
		row := conn.QueryRowContext(ctx, playbook.Config.DatabaseNflow.QueryGetTemplateCount, name)
		var count int
		err = row.Scan(&count)
		if err != nil {
			log.Println(err)
		...
		if count > 0 {
			_, err = conn.ExecContext(ctx, playbook.Config.DatabaseNflow.QueryUpdateTemplate, content, name)
		} else {
			_, err = conn.ExecContext(ctx, playbook.Config.DatabaseNflow.QueryInsertTemplate, content, name)
		}
		if err != nil {
			log.Println(err)
			return c.JSON(http.StatusNotFound, map[string]interface{}{"message": "Not Found"})
	...
	gNFlow.PUT("/templates", func(c echo.Context) error {
		jsonValue := make(map[string]interface{})
		err := c.Bind(&jsonValue)
		if err != nil {
			log.Println(err)
