...
import (
	"github.com/arturoeanton/nFlow/pkg/redistore"
...
func NewRedisStore(size int, network, address, password string, options []redis.DialOption, keyPairs ...[]byte) (RedisStore, error) {
	store, err := redistore.NewRediStore(size, network, address, password, options, keyPairs...)
	if err != nil {
		return nil, err
	}
